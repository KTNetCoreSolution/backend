...
@RestController
@RequestMapping("${api.base.path}/oper/userpasswordinitmng")
@RequiredArgsConstructor
@io.swagger.v3.oas.annotations.tags.Tag(name = "3.시스템관리 > 사용자비밀번호초기화관리", description = "사용자비밀번호초기화를 관리하는 API")
public class OperUserPasswordInitMngController {
    private static final Logger logger = LoggerFactory.getLogger(OperUserPasswordInitMngController.class);
    ...
    @CommonApiResponses
    @PostMapping("/list")
    public ResponseEntity<ApiResponseDto<List<Map<String, Object>>>> userPasswordInitMngList(
            @RequestBody Map<String, Object> request,
            HttpServletRequest httpRequest
    ) {
        String rptCd = "USERPASSWORDINITMNG";
        String jobGb = "GET";
    ...
    @CommonApiResponses
    @PostMapping("/save")
    public ResponseEntity<ApiResponseDto<List<Map<String, Object>>>> userPasswordInitMngSave(
            @RequestBody Map<String, Object> request,
            HttpServletRequest httpRequest
    ) {
        String rptCd = "USERPASSWORDINITMNGTRAN";
        String jobGb = "SET";
